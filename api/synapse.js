/* SYNAPSE — the channel CORTEX and JARVIS talk over. Backed by a secret
   gist so two separate Vercel apps (orrery + jarvis-talk) share one bus
   with zero new vendors. Cadence is conversational (seconds), so gist
   API latency and last-write-wins races are acceptable by design. */

const FROM_OK = new Set(['CORTEX', 'JARVIS', 'HUMAN']);
const KEEP = 40;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      const msg = { id: Date.now() + '-' + Math.random().toString(36).slice(2, 6), ts: new Date().toISOString(), from, text };
